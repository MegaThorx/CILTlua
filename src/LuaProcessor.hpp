...
public:
	...
	*/
	static std::string methodSigToString(MethodSignature *sig)
	{
		return std::string(sig->namespaceName) + "_" + sig->className + "_" + sig->methodName;
	}

	static std::string nullToString()
	{
		return "nil";
	}

	LuaProcessor(const std::unordered_map<std::string, std::string> &predefinedReferences, const char *predefinedCode) : 
		m_predefinedReferences(predefinedReferences),
		m_predefinedCode(predefinedCode)
	{
		addTransCompData("-- This file was generated by translating CIL to lua using CILTlua (https://github.com/DEC05EBA/CILTlua)\n");
		addTransCompData(predefinedCode);
		cil::Value::setMethodRefToStringCallbackFunc(methodSigToString);
		cil::Value::setNullToStringCalllbackFunc(nullToString);
	}
	...
	void onMethodCall(cil::Value lhsVar, const std::string &namespaceName, const std::string &className, const std::string &methodName, const std::vector<cil::Value> &args)
	{
		...
		{
			const std::string *predefinedFunction = getPredefinedReference(namespaceName + "." + className + "." + methodName);
	...
	{
		const std::string &staticFieldName = namespaceName + "." + className + "." + field;
		const std::string *staticFieldRef = getPredefinedReference(staticFieldName);

		if(staticFieldRef)
			addTransCompData("local " + resultVar.toString() + " = " + *staticFieldRef + "\n");
		else
			addTransCompData("local " + resultVar.toString() + " = " + staticFieldName + "\n");
	}
...
private:
	...
	const std::string* getPredefinedReference(const std::string &key)
	{
		std::unordered_map<std::string, std::string>::const_iterator it = m_predefinedReferences.find(key);
		if(it != m_predefinedReferences.cend())
			return &it->second;

		return nullptr;
	}
private:
	std::unordered_map<std::string, std::string> m_predefinedReferences;
	const char *m_predefinedCode;
};
